...
from ariadne.github import ChecksFacts, GhClient, PullRequestFacts
from ariadne.pipeline.gates import GateError, PipelineController
from ariadne.pipeline.task_parser import InvalidTask, parse_task
from ariadne.preview import PreviewFacts
from ariadne.state import StateStore
from ariadne.models import Provider, SessionStatus, TurnState
...
class GitHubClientTests(unittest.TestCase):
    ...
    def test_failed_checks_are_not_green(self):
        ...
        with patch("ariadne.github.subprocess.run", return_value=completed):
            facts = GhClient().get_checks(7)
...
class GateTests(unittest.TestCase):
    ...
    def test_task_scope_includes_committed_changes_since_approval(self):
        task_path = self.root / "worktree" / "docs" / "tasks" / "TASK-safe.md"
        task_path.parent.mkdir(parents=True)
        task_path.write_text(
            """# Objective
Safely update documentation.

# Allowed files
- `docs/**`

# Forbidden zones
Do not modify root files.
...
""",
        ...
        forbidden = self.root / "worktree" / "forbidden.txt"
        forbidden.write_text("outside task scope\n", encoding="utf-8")
        subprocess.run(
            ["git", "-C", str(self.root / "worktree"), "add", "forbidden.txt"],
            check=True,
            stdout=subprocess.DEVNULL,
        )
        subprocess.run(
            [
                ...
                "-c", "user.email=harness@example.invalid", "commit", "-m", "unsafe",
            ],
            check=True,
            stdout=subprocess.DEVNULL,
        )
