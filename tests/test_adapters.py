...
from ariadne.adapters import ClaudeAdapter, CodexAdapter
from ariadne.adapters.base import AdapterError
from ariadne.adapters.sessions import ModelSwitchError, ProviderSessionController
from ariadne.filesystem import StateLayout
from ariadne.models import Provider, ProviderSessionStatus, TurnState
...
class AdapterCommandTests(unittest.TestCase):
    ...
    def test_claude_commands_use_dont_ask_and_explicit_tools(self):
        ...
        review = adapter.review_command(model="a", prompt="review this", base_ref="origin/main")
        review_tools = review[review.index("--allowed-tools=Read,Bash(git status),Bash(git diff *),Bash(git log *),Bash(git show *),Bash(git rev-parse *),Bash(git ls-files *)")]
        self.assertNotIn("Edit", review_tools)
        self.assertNotIn("Write", review_tools)
        self.assertNotIn("--resume", review)
