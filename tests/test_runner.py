...
from ariadne.redaction import Redactor
from ariadne.runner import RunnerError, TurnRunner, build_child_environment
...
from ariadne.transcript import read_delta
...
class RunnerTests(unittest.TestCase):
    ...
    def test_hermes_turn_uses_the_same_durable_runner_and_blocks_review_on_failed_gate(self):
        session = self.state.get_session("S-0001")
        provider = self.state.list_provider_sessions(session.id)[0]
        hermes = self.state.create_turn(
            provider_session_id=provider.id,
            owner_message_id="task:S-0001:1",
        ...
        self.state.enqueue_turn(hermes.id)
        # The setup turn is active; finish it so Hermes owns the single slot.
        self.state.finalize_turn(self.turn.id, state=TurnState.SUCCEEDED, exit_code=0)
        claimed = self.state.claim_next()
        self.assertEqual(claimed.id, hermes.id)
        result = TurnRunner(state=self.state, layout=self.layout).run(
            hermes.id,
            [sys.executable, "-c", "print('Hermes completed')"],
            cwd=self.worktree,
            completion_validator=lambda: "TASK verification failed",
        )
        self.assertEqual(result["state"], TurnState.FAILED.value)
        final = self.state.get_turn(hermes.id)
        self.assertEqual(final.state, TurnState.FAILED)
        self.assertIsNotNone(final.result_path)
        self.assertEqual(self.state.get_session(session.id).status, SessionStatus.NEEDS_OWNER)
...
class CoordinatorTests(unittest.TestCase):
    ...
    def test_only_one_transient_unit_is_started(self):
        ...
        self.assertTrue(self.systemd.started[0].unit_name.startswith("ariadne-turn-S-0001-T-"))
