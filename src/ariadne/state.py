...
class StateStore:
    ...
    def _migrate(self) -> None:
        with self._lock:
            ...
            try:
                ...
                if current < 6:
                    self._connection.execute(
                        "ALTER TABLE turns ADD COLUMN execution_kind TEXT NOT NULL DEFAULT 'provider' "
                        "CHECK (execution_kind IN ('provider', 'hermes', 'review'))"
                    )
                    self._connection.execute("ALTER TABLE pipeline_runs ADD COLUMN task_path TEXT")
                    self._connection.execute("ALTER TABLE pipeline_runs ADD COLUMN task_hash TEXT")
                    self._connection.execute("ALTER TABLE pipeline_runs ADD COLUMN task_turn_id TEXT")
                    current = 6
                if current < 7:
                    self._connection.execute(
                        "ALTER TABLE pipeline_runs ADD COLUMN task_baseline_json TEXT"
                    )
                    current = 7
                if current < 8:
                    self._connection.execute(
                        "ALTER TABLE turns ADD COLUMN success_session_status TEXT"
                    )
                    current = 8
                if current < 9:
                    self._connection.execute(
    ...
    def _turn_from_row(row: sqlite3.Row) -> Turn:
        return Turn(
            ...
            execution_kind=TurnKind(row["execution_kind"]),
            success_session_status=(
                SessionStatus(row["success_session_status"])
                if row["success_session_status"]
                else None
            ),
    ...
    def _pipeline_from_row(row: sqlite3.Row) -> PipelineRun:
        return PipelineRun(
            ...
            task_path=_optional_path(row["task_path"]),
            task_hash=row["task_hash"],
            task_turn_id=row["task_turn_id"],
            task_baseline_json=row["task_baseline_json"],
            task_start_head_sha=row["task_start_head_sha"],
    ...
    def get_or_create_pipeline_session(
        ...
        *,
        ...
        branch_prefix = str(branch_prefix).strip()
        if not branch_prefix.endswith("/") or ".." in branch_prefix or not branch_prefix[:-1]:
            raise StateError("pipeline branch prefix is invalid")
        with self._transaction(immediate=True) as conn:
            existing = conn.execute(
                "SELECT * FROM harness_sessions WHERE source_message_id = ?",
                (source_message_id,),
    ...
    def create_turn(
        ...
        *,
        ...
        attempt: int = 1,
        execution_kind: TurnKind = TurnKind.PROVIDER,
        success_session_status: SessionStatus | None = None,
    ) -> Turn:
        requested_model = requested_model.strip()
        configured_model = configured_model.strip()
        requested_effort = requested_effort.strip().lower()
        ...
        if not isinstance(execution_kind, TurnKind):
            raise StateError("turn execution kind is invalid")
        if success_session_status is not None and not isinstance(success_session_status, SessionStatus):
            raise StateError("turn success session status is invalid")
        with self._transaction(immediate=True) as conn:
            provider_row = conn.execute(
                "SELECT * FROM provider_sessions WHERE id = ?", (provider_session_id,)
            ).fetchone()
            ...
            try:
                ...
                (id, provider_session_id, owner_message_id, requested_model,
                 configured_model, requested_effort, configured_effort,
                 reported_model, execution_kind, state, attempt, input_path, success_session_status, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, NULL, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        turn_id,
                        ...
                        success_session_status.value if success_session_status else None,
                        now,
                        now,
                    ),
                )
    ...
    def queue_task_turn(
        ...
        *,
        ...
        """Atomically record an approved TASK and enqueue its Hermes turn.

        The active strong-model provider is retained as the owning session,
        but the turn is explicitly marked ``hermes`` so the transient runner
        cannot accidentally invoke Codex or Claude for the implementation.
        """

        task_path = Path(task_path)
        if not task_path.is_absolute() or not task_hash or not task_baseline_json or not task_start_head_sha:
            raise StateError("approved task path, hash, baseline, and start head are required")
        with self._transaction(immediate=True) as conn:
            session = self._require_session(conn, harness_session_id)
            if SessionStatus(session["status"]) not in {
                SessionStatus.PLAN_APPROVED,
                SessionStatus.NEEDS_OWNER,
            }:
                raise InvalidTransition("TASK can be queued only after PLAN approval or owner intervention")
            provider = conn.execute(
                "SELECT * FROM provider_sessions WHERE id = ?", (provider_session_id,)
            ).fetchone()
            if provider is None or provider["harness_session_id"] != harness_session_id:
                raise StateError("TASK provider session does not belong to this Harness session")
            pipeline = conn.execute(
                "SELECT * FROM pipeline_runs WHERE harness_session_id = ?", (harness_session_id,)
            ).fetchone()
            if pipeline is None:
                raise NotFoundError("pipeline run not found")
            number = self._next_sequence(conn, "turn_sequence")
            turn_id = f"T-{number:04d}"
            attempt = int(
                conn.execute(
                    "SELECT COUNT(*) FROM turns WHERE provider_session_id = ? AND execution_kind = 'hermes'",
                    (provider_session_id,),
                ).fetchone()[0]
            ) + 1
            owner_message_id = f"task:{harness_session_id}:{task_hash[:16]}:{attempt}"
            now = _utc_now()
            conn.execute(
                """
                INSERT INTO turns
                (id, provider_session_id, owner_message_id, requested_model,
                 configured_model, requested_effort, configured_effort,
                 reported_model, execution_kind, state, attempt, input_path, created_at, updated_at)
                VALUES (?, ?, ?, 'hermes', 'hermes', 'none', 'none', NULL, 'hermes', 'queued', ?, NULL, ?, ?)
                """,
                (turn_id, provider_session_id, owner_message_id, attempt, now, now),
            )
            ordinal = int(conn.execute("SELECT COALESCE(MAX(ordinal), 0) + 1 FROM queue").fetchone()[0])
            conn.execute(
                "INSERT INTO queue(turn_id, ordinal, queued_at) VALUES (?, ?, ?)",
            ...
            conn.execute(
                ...
                SET task_path = ?, task_hash = ?, task_turn_id = ?, task_baseline_json = ?,
                    task_start_head_sha = ?, updated_at = ?
                WHERE harness_session_id = ?
                """,
                (
                    str(task_path), task_hash, turn_id, task_baseline_json,
                    task_start_head_sha, now, harness_session_id,
                ),
            )
            conn.execute(
                "UPDATE harness_sessions SET status = ?, updated_at = ? WHERE id = ?",
                (SessionStatus.TASK_RUNNING.value, now, harness_session_id),
            )
            row = conn.execute("SELECT * FROM turns WHERE id = ?", (turn_id,)).fetchone()
        assert row is not None
        return self._turn_from_row(row)
    ...
    def _default_unit_name(row: sqlite3.Row, session_id: str) -> str:
        return f"ariadne-turn-{session_id}-{row['id']}"
        ...
        *,
        ...
        with self._transaction(immediate=True) as conn:
            ...
            if current.terminal:
                ...
                kind = TurnKind(row["execution_kind"])
                requested_success_status = (
                    SessionStatus(row["success_session_status"])
                    if row["success_session_status"]
                    else None
                )
                if state is TurnState.SUCCEEDED and requested_success_status is not None:
                    next_session_status = requested_success_status
                elif kind in {TurnKind.HERMES, TurnKind.REVIEW}:
                    next_session_status = {
                        TurnState.SUCCEEDED: SessionStatus.REVIEW_PENDING,
                        TurnState.FAILED: SessionStatus.NEEDS_OWNER,
                        TurnState.INTERRUPTED: SessionStatus.NEEDS_OWNER,
                        TurnState.CANCELLED: SessionStatus.NEEDS_OWNER,
                    }[state]
                else:
                    next_session_status = {
    ...
    def create_pipeline_run(self, harness_session_id: str, **values) -> PipelineRun:
        ...
        with self._transaction(immediate=True) as conn:
            ...
            cursor = conn.execute(
                ...
                (harness_session_id, plan_path, plan_hash, base_sha, task_path, task_hash, task_turn_id, task_baseline_json, task_start_head_sha, head_sha,
                ...
                (
                    ...
                    normalized.get("task_path"),
                    normalized.get("task_hash"),
                    normalized.get("task_turn_id"),
                    normalized.get("task_baseline_json"),
                    normalized.get("task_start_head_sha"),
