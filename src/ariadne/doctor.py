...
def run_doctor(*, env_path: Path) -> tuple[bool, tuple[DoctorCheck, ...]]:
    ...
    for path, label in (
        (config.codex_bin, "Codex executable"),
        (config.claude_bin, "Claude executable"),
        (config.hermes_bin, "Hermes executable"),
    ):
    ...
    repo = config.project_repo.resolve()
    root = config.worktree_root.resolve()
    inside_forbidden = False
    for forbidden in (repo, *config.profile.forbidden_roots(repo)):
        try:
    ...
    try:
        ...
        turn_units = [
            line.split()[0]
            for line in active_units.stdout.splitlines()
            if line.startswith(("ariadne-turn-", "dev-pipeline-turn-"))
        ]
