...
def parse_control(text: str) -> ControlCommand | None:
    ...
    if name not in _COMMANDS:
        raise CommandParseError("unknown control command; use !status, !stop, !approve, !task, !reject, !resume, or !accept")
    if name in {"status", "stop"} and args:
        raise CommandParseError(f"!{name} takes no arguments")
    if name in {"model", "effort", "provider", "approve", "task", "resume"} and len(args) != 1:
...
def command_help() -> str:
    return "可用命令：!status · !stop · !approve <PLAN-id> · !task <TASK-id> · !reject <反馈> · !resume <S-####> · !accept <PR#> <full-head-SHA>；模型和推理强度仅在新 Thread 的配置卡中一次性固定。"
